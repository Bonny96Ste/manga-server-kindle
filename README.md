...
The repository stores source files, not generated release archives or private configuration.
...
```text
dist/mangabridge-server-v1.0.2.zip
dist/mangabridge-kindle-usb-v1.0.5.zip
dist/SHA256SUMS.txt
```

Upload those generated ZIPs to **GitHub Releases**, not to the normal source tree.
...
- `.env`
- Real API tokens or passwords
- `/mnt/us/mangabridge/config.lua`
- `server/data/`
- `server/venv/`
- Downloaded PDFs or page images
- Kindle cache, logs, diagnostics, or `.part` files
- Generated `dist/` archives in the source branch

Before the first push, search the repository for any token, password, LAN address, username, or downloaded content that may have been copied during testing.
...
git add .
git status
git commit -m "Initial MangaBridge release"
git branch -M main
git remote add origin git@github.com:YOUR_ACCOUNT/mangabridge.git
git push -u origin main
```

Create a tag and release after building the archives:
...
Attach the files from `dist/` to the GitHub Release.
